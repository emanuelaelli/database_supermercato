...
<?php
	if(!isset($_POST['udata'])){
        ...
        if (!$conn){
            ...
            if (!$result2) {
                ...
                print("<input id=\"udata\" type=\"submit\" name=\"udata\" value=\"Vedi impiegati\">");
                print("</form>");
    ...
    if(isset($_POST['udata']) and $_POST['udata']=='View') {
		...
		if (!$conn){
            ...
            //echo "Connessione riuscita."."<br/>";
            $query="SELECT  cf, datadinascita, cap, via, numciv, città, nome, cognome, email, mansione, numturno, numlivello, nomereparto, viasupermercato, numcivsupermercato 
    ...
    } else if (isset($_POST['udata']) and $_POST['udata']=='Vedi impiegati') { //SCELTA IMPIEGATO
        $conn = pg_connect("host=localhost port=5432 dbname=progettosupermercato user=postgres password=BDDWLab20");
        if (!$conn){
            echo 'Connessione al database fallita.';
            exit();
            ...
            if (!$result3) {
                ...
                print("<select name=\"lavora\" onchange=\"myfunction();\">");
                while ($row = pg_fetch_array($result3)) {
                    print("<option value=\"".htmlspecialchars($row["cfimpiegato"])."\">");
                    echo $row["cfimpiegato"];
                    print("</option>");					
                };
		...
		if (!$conn){
            ...
            if (!isset($_POST['udata'])) {
                ...
                echo preg_last_error($conn);
                exit();
                //die('Connessione al database fallita.');
            };
			//echo "Connessione riuscita."."<br/>";
            $query="SELECT  cf, datadinascita, cap, via, numciv, città, nome, cognome, email, mansione, numturno, numlivello 
            FROM Impiegato JOIN Lavora ON cf = cfimpiegato 
            WHERE datafine ISNULL AND cf='".$_POST['lavora']."'";
            //echo $query;
            $result =  pg_query($conn, $query);
